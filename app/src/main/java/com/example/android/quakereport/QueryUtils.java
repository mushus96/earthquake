...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject raw_data= new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray feature= raw_data.optJSONArray("features");

            for(int i=0;i<feature.length();i++) {

                JSONObject single = feature.optJSONObject(i);
                JSONObject prop = single.optJSONObject("properties");
                String url = prop.getString("url");
                double mag = prop.getDouble("mag");
                String loc= prop.optString("place");
                long date= Long.parseLong(prop.optString("time"));

                earthquakes.add(new Earthquake(mag,loc,date,url));
            }
